...
function normalizeColorFunctions(value: string, doc: Document): string {
  ...
  const resolved = value.replace(/oklch\([^)]*\)/g, (colorExpr) => {
    swatch.style.color = '';
    swatch.style.color = colorExpr;
    const rgb = doc.defaultView?.getComputedStyle(swatch).color;
    return rgb && rgb !== '' ? rgb : colorExpr;
  });
...
function patchUnsupportedColors(doc: Document): void {
  const targets = doc.querySelectorAll<HTMLElement | SVGElement>('*');
  const colorProps = [
    'color',
    'background-color',
    'border-top-color',
  ...
  for (const el of targets) {
    ...
    for (const prop of colorProps) {
      ...
      const normalized = normalizeColorFunctions(value, doc);
...
export async function captureScreenshot(element: HTMLElement): Promise<void> {
  ...
  const canvas = await html2canvas(element, {
    ...
    logging: false,
    onclone: (doc) => {
      patchUnsupportedColors(doc);
    }
  });
  const dataUrl = canvas.toDataURL('image/png');
  const a = document.createElement('a');
  a.href = dataUrl;
  a.download = `mydevice-screenshot-${Date.now()}.png`;
  a.click();
}
